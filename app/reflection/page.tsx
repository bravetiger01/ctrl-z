...
async function analyzeRegret(regret: string) {
  await sleep(2000); // 👈 force minimum loading time (2s)

  const res = await fetch("http://localhost:3000/api/analyze", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ regret }),
    cache: "no-store",
  });
...
// export default async function ReflectionPage({searchParams}: {searchParams: {regret: string}}) {

//   const { regret } = await searchParams;

//   if (!regret) {
//     return <p>No regret provided</p>;
//   }
...
export default async function ReflectionPage({
  ...
  searchParams: Promise<{ regret?: string }>;
}) {
  const { regret } = await searchParams;
  if (!regret) return <p>No regret provided</p>;

  const analysis = await analyzeRegret(regret);

  return (
    <main>
      <h1>Reflection</h1>
      <p>{analysis.whatWentWrong}</p>
      <p>{analysis.lessonLearned}</p>
      <p>{analysis.betterFuture}</p>
    </main>
  );
}
